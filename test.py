...
prepare_to_kafka_df.writeStream.trigger(availableNow=True).outputMode("append").format(
    "console"
).option("checkpointLocation", "/tmp/checkpoints-2").start()
...
query = (
    ...
    .option("topic", f'{kafka_config["my_name"]}_temperature_alerts')
    .option("kafka.security.protocol", "SASL_PLAINTEXT")
    .option("kafka.sasl.mechanism", "PLAIN")
    .option(
        "kafka.sasl.jaas.config",
